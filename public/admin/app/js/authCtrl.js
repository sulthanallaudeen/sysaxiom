...
app.controller('profileControl', function ($scope) {
});
...
app.controller('authCtrl', function ($scope, $rootScope, $routeParams, $location, $http, Data) {
    ...
    $scope.doLogin = function ()
    {
        ...
        $scope.data.show = false;
        $scope.data.show = true;
        $http.post('authLogin', {"email": email, "password": password})
                .success(function (status, data, response, header) {
                    if (status.success == 1)
                    {
                        $location.path('home');
    ...
    {
        profileUserName = $scope.userdetail.profileUserName;
        profileEmail = $scope.userdetail.profileEmail;
        profilePhone = $scope.userdetail.profilePhone;
        $scope.data.show = false;
        $scope.data.show = true;
        $http({
            url: 'updateProfile',
            method: "GET",
            params: {name: profileUserName, email: profileEmail, phone: profilePhone}
        }).success(function (status, data, response, header) {
